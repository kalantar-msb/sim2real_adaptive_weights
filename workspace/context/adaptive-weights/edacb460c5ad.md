# Translation Context
Scenario: adaptive-weights | inference-sim@b05ce8f | llm-d-inference-scheduler@2d2799f
...
## Production Interfaces (from sigs.k8s.io/gateway-api-inference-extension@v1.4.0)
...
const (
	// Affinity prefers endpoints with existing locality (kv-cache, session state).
	Affinity ScorerCategory = "Affinity"
	// Distribution prefers spreading requests evenly across all endpoints.
	Distribution ScorerCategory = "Distribution"
	// Balance is balanced between Affinity and Distribution.
	Balance ScorerCategory = "Balance"
)

// Scorer defines the interface for scoring a list of pods based on context.
// Scorers must score pods with a value within [0,1] where 1 is highest.
// Values > 1 treated as 1; values < 0 treated as 0.
...
// Filter defines the interface for filtering a list of pods.
...
### pkg/epp/framework/interface/scheduling/types.go (key types)

```go
// Endpoint is what Score() receives — one per candidate pod.
...
// Metrics fields used by sub-scorers:
//   KVCacheUsagePercent float64  (0-1)   — used by kv-cache-utilization-scorer
//   WaitingQueueSize    int              — used by queue-scorer

// LLMRequest carries the parsed request fields.
...
// HandlePlugins defines APIs to work with instantiated plugins
type HandlePlugins interface {
	Plugin(name string) Plugin        // returns the named plugin instance
...
// Usage: ppcScorer, err := plugin.PluginByType[scheduling.Scorer](handle, "ppc")
...
### pkg/epp/framework/interface/plugin/registry.go

```go
package plugin

// FactoryFunc is the factory function signature for all plugins.
type FactoryFunc func(name string, parameters json.RawMessage, handle Handle) (Plugin, error)

// Register registers a plugin factory. Call once at startup.
func Register(pluginType string, factory FactoryFunc) {
	Registry[pluginType] = factory
}

// Registry maps plugin type strings to factory functions.
var Registry map[string]FactoryFunc = map[string]FactoryFunc{}
```

### pkg/epp/framework/interface/scheduling/cycle_state.go

```go
// CycleState provides per-request storage for plugins.
// Most scorers can ignore it (pass through to sub-scorers unchanged).
...
## Example Plugin: scorer/load_aware.go (from llm-d-inference-scheduler)

This is the simplest scorer in the target repo — use as structural template.
...
// LoadAwareFactory is the factory function — signature matches plugin.FactoryFunc.
...
// Category satisfies scheduling.Scorer
func (s *LoadAware) Category() scheduling.ScorerCategory { return scheduling.Distribution }

// Score satisfies scheduling.Scorer — returns map[Endpoint]float64 in [0,1]
...
## Plugin Registration (pkg/plugins/register.go)

The new adaptive-routing-scorer must be added here alongside existing registrations:
...
func RegisterAllPlugins() {
	...
	// ... other registrations ...
	// ADD: plugin.Register(scorer.AdaptiveRoutingScorerType, scorer.AdaptiveRoutingScorerFactory)
}
```

**Key pattern**: `plugin.Register("type-string", PackageFactory)` — the type string matches
what appears in `EndpointPickerConfig` YAML. The new scorer type must be:
`"adaptive-routing-scorer"` (matching `config.md`).

---

## Algorithm Source (algorithms/algorithm_adaptive.go)

The pure-Go algorithm to translate into the plugin. No BLIS deps — just `math`.
...
type ScorerOutput struct {
	Scores map[string]float64 // endpoint ID → score [0,1]
}

type AdaptiveWeights struct {
	BaseWeights []float64 // e.g. [3,2,2] normalized to [3/7, 2/7, 2/7]
...
func (aw *AdaptiveWeights) Blend(scorerOutputs []ScorerOutput, endpointIDs []string) map[string]float64 {
	n := len(aw.BaseWeights)
	k := float64(len(endpointIDs))
	alpha := 2.0 / (k + 1.0)
	...
	// Step 2: adaptive weights = base_weight × entropy^(k/2), renormalized
	exponent := k / 2
