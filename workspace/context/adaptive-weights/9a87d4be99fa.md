# Translation Context
Scenario: adaptive-weights | inference-sim@unknown | llm-d-inference-scheduler@2d2799f
...
func (tn TypedName) String() string { return tn.Type + "/" + tn.Name }
```

### plugin.Handle (handle.go)
```go
...
### scheduling.CycleState (cycle_state.go)
```go
// CycleState provides a mechanism for plugins to store and retrieve arbitrary data.
type CycleState struct {
	storage sync.Map // key: StateKey, value: StateData
}
...
### scheduling.LLMRequest (types.go)
```go
...
type Metrics struct {
	...
	MaxActiveModels         int
	RunningRequestsSize     int  // requests currently executing (batch size)
	WaitingQueueSize        int  // requests waiting in queue
	KVCacheUsagePercent     float64  // 0–100
...
### Scorer Factory signature
Every scorer factory registered with `plugin.Register` must match:
```go
func MyFactory(name string, rawParameters json.RawMessage, handle plugin.Handle) (plugin.Plugin, error)
```

---

## Example Plugin: load_aware.go
...
const LoadAwareType = "load-aware-scorer"
const QueueThresholdDefault = 128
...
Pattern: `plugin.Register(TypeConstant, FactoryFunction)` — call once per plugin type at startup.

---

## Existing adaptive_routing.go in target repo

The target repo already has `pkg/plugins/scorer/adaptive_routing.go` implementing `AdaptiveRoutingScorer`.
This is the previous sim2real translation. The current task is to update or replace it with a new translation.

Key details of the existing implementation:
- Type: `"adaptive-routing-scorer"` (`AdaptiveRoutingScorerType`)
- Factory: `AdaptiveRoutingScorerFactory`
- Registered in `register.go` as `scorer.AdaptiveRoutingScorerType` → `scorer.AdaptiveRoutingScorerFactory`
- The plugin is a **delegation scorer**: it calls sub-scorers (e.g. PPC, queue, KV) via `plugin.Handle`
  and blends their outputs with entropy-adaptive weights
- Parameters: `scorerNames []string`, `baseWeights []float64`, `mode string` ("adaptive"|"control")
- Category: `scheduling.Balance`
- Test file: `pkg/plugins/scorer/adaptive_routing_test.go`
